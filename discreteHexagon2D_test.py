...
fault_vec1 = np.array([[-2, 2]]).T
fault_vec2 = np.array([[-2,-3]]).T
...
p_hat = deepcopy(p_est)                                                     # Reported position of agents
p_true = deepcopy(p_hat)                                                    # True position of agents
est_pos_history = [np.zeros((dim, (n_iter))) for i in range(num_agents)]    # Value of p at each iteration of algorithm
y = meas_model(p_true, x_star)                                              # Phi(p_hat + x_hat), true interagent measurement
...
print("Faulty Agent Vector 1:", fault_vec1.flatten())
print("Faulty Agent Vector 2:", fault_vec2.flatten())
...
for outer_i in tqdm(range(n_scp), desc="SCP Loop ", leave=False):
    ...
    # Check outer_i value for discrete network configuration change

    p_hat = deepcopy(p_true)
    if (outer_i >= (n_scp / 2)):
        p_hat[faulty_id] += fault_vec2
    elif (outer_i >= 0):
        p_hat[faulty_id] += fault_vec1
    ...
    for inner_i in tqdm(range(n_admm), desc="ADMM Loop", leave=False):
        ...
        for id, agent in enumerate(agents):
            ...
            new_x = deepcopy(agent.x_bar.flatten()) + deepcopy(x_star[id].flatten())
            x_history[id][:, inner_i + outer_i*n_admm] = deepcopy(new_x.flatten())
        ...
        for id, agent in enumerate(agents):
            # Reported position
            est_pos_history[id][:, inner_i + outer_i*n_admm] = deepcopy(p_hat[id].flatten())
...
plt.ylabel("y")
for agent_id, agent in enumerate(agents):
    plt.scatter(est_pos_history[agent_id][0, -1], est_pos_history[agent_id][1, -1], marker='o', c='c', label="Reported")
...
fname = "fig/2D-DiscreteHexagon/pos2D_ani_" + dt_string + ".gif"
