...
if __name__ == '__main__':
    num_trials = 5
    rho_arr = [0.1, 0.25, 0.5, 0.75, 1]
    warm_start_arr = [True, False]
    tot_rmse_arr = np.zeros((len(warm_start_arr), len(rho_arr)))
    for i, warm_start in enumerate(warm_start_arr):
        for j, rho in enumerate(rho_arr):
            print(f"Warm Start: {warm_start}\t Rho = {rho}")
            for trial_num in range(num_trials):
                print(f"\tTrial {trial_num}...")
                try:
                    trial_avg_rmse = main_func(rho, warm_start)
                except cp.SolverError:
                    trial_avg_rmse = 300
                plt.close("all")
                tot_rmse_arr[i, j] += (trial_avg_rmse)

    print(f"Trials completed. Results: ")
    print("========================================================")
    for i, warm_start in enumerate(warm_start_arr):
        for j, rho in enumerate(rho_arr):
            print(f"Warm Start: {warm_start}; \tRho: {rho}; \tAvg RMSE: {tot_rmse_arr[i, j] / num_trials}")
    print("========================================================\n")