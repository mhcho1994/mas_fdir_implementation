...
n_scp           =   2  # Number of SCP iterations
...
# added -> Checking Laplacian
A           =   np.zeros((len(agents),len(agents)))
for _, edge in enumerate(edges):
    A[edge[0],edge[1]] = 1

L           =   -A
for row_id in range(len(agents)):
    L[row_id,row_id] = np.sum(A[row_id,:])
...
print("Laplacian: \n")
print(L)
print("Algebraic connectivity:", np.sort(eigval)[1])
